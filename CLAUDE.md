...
- All custom CSS classes MUST use BS\_ prefix
...
- Never use generic names like .card, .button, .wrapper without BS\_ prefix
...
- Project uses JavaScript only — no TypeScript. All files are .jsx not
  .tsx. No type annotations, no interfaces, no type imports, no JSDoc
  types. Plain JavaScript only.
